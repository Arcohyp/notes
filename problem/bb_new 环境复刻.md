...
## 6.子机配置
最主要的是对rs_yolact_aruco_f/_b.launch文件的修改，一定要确保注意CMakeList.txt文件当中的修改同步

这两个文件最后启动的节点本质上其实是一致的，但我们需要修改不同的名字以区别它们（但代码中并没有这么做，而是复制了一份，用不同的type来区别了，这在CMakeList.txt中可以进行修改）

除此之外，还有对三台机器的.bashrc中内容的修改，其中最主要的就是要明确本机IP和主机IP，三台都需要配置这个

运行的过程中会出现data的丢失，暂时没有问题
